...
const requireModule = module$1.createRequire((typeof document === 'undefined' ? new (require('u' + 'rl').URL)('file:' + __filename).href : (document.currentScript && document.currentScript.src || new URL('index.cjs', document.baseURI).href)));
            ...
            : new Proxy(currentItem, {
                ...
                construct(target, argArray, newTarget) {
                    return handleHookResult(hook({
                        kind: "__$$_PROXY_CONSTRUCT" /* PolicyTrapKind.CONSTRUCT */,
                        policy,
                        newTarget,
                        argArray,
                        target,
                ...
                apply(target, thisArg, argArray = []) {
                    return handleHookResult(hook({
                        kind: "__$$_PROXY_APPLY" /* PolicyTrapKind.APPLY */,
                        policy,
                        thisArg,
                        argArray,
                        target,
                ...
                get(target, property, receiver) {
                    ...
                    return handleHookResult(hook({
                        kind: "__$$_PROXY_GET" /* PolicyTrapKind.GET */,
                        policy,
...
function stringifyPolicyTrapKindOnPath(kind, path) {
    switch (kind) {
        case "__$$_PROXY_GET" /* PolicyTrapKind.GET */:
            return `get ${path}`;
        case "__$$_PROXY_APPLY" /* PolicyTrapKind.APPLY */:
            return `${path}(...)`;
        case "__$$_PROXY_CONSTRUCT" /* PolicyTrapKind.CONSTRUCT */:
            return `new ${path}(...)`;
    }
}
...
const NETWORK_MAP = {
    ...
    http2: {
        connect: {
            ["__$$_PROXY_APPLY" /* PolicyTrapKind.APPLY */]: true
        },
        createSecureServer: {
            ["__$$_PROXY_APPLY" /* PolicyTrapKind.APPLY */]: true
        },
    ...
    https: {
        ...
        Server: {
            ["__$$_PROXY_CONSTRUCT" /* PolicyTrapKind.CONSTRUCT */]: true
        },
        globalAgent: {
            destroy: {
                ["__$$_PROXY_APPLY" /* PolicyTrapKind.APPLY */]: true
...
const NONDETERMINISTIC_MAP = {
    ...
    Date: {
        ...
        ["__$$_PROXY_CONSTRUCT" /* PolicyTrapKind.CONSTRUCT */]: (...args) => args.length === 0 && !(args[0] instanceof Date)
    }
};
...
const IO_MAP = {
    ...
    fs: {
        readFile: {
            ["__$$_PROXY_APPLY" /* PolicyTrapKind.APPLY */]: "read"
        },
        readFileSync: {
            ["__$$_PROXY_APPLY" /* PolicyTrapKind.APPLY */]: "read"
        },
        ...
        createWriteStream: {
            ["__$$_PROXY_APPLY" /* PolicyTrapKind.APPLY */]: "write"
        },
        copyFile: {
            ["__$$_PROXY_APPLY" /* PolicyTrapKind.APPLY */]: "write"
        },
...
const PROCESS_MAP = {
    ...
    process: {
        exit: {
            ["__$$_PROXY_APPLY" /* PolicyTrapKind.APPLY */]: "exit"
        }
    },
    // Everything inside child_process is just one big violation of this policy
    "node:child_process": "child_process",
    child_process: {
        ["__$$_PROXY_APPLY" /* PolicyTrapKind.APPLY */]: "spawnChild"
    },
    "node:cluster": "cluster",
    cluster: {
        Worker: {
            ["__$$_PROXY_CONSTRUCT" /* PolicyTrapKind.CONSTRUCT */]: "spawnChild"
        }
    }
};
...
function getDotPathFromNode(options) {
    ...
    else if (((_b = typescript.isTypeAssertionExpression) === null || _b === void 0 ? void 0 : _b.call(typescript, node)) || (!("isTypeAssertionExpression" in typescript) && typescript.isTypeAssertion(node))) {
        return getDotPathFromNode({ ...options, node: node.expression });
    }
...
function getInnerNode(node, typescript) {
    ...
    if (typescript.isParenthesizedExpression(node))
        ...
        return getInnerNode(node.expression, typescript);
    else if (((_a = typescript.isTypeAssertionExpression) === null || _a === void 0 ? void 0 : _a.call(typescript, node)) || (!("isTypeAssertionExpression" in typescript) && typescript.isTypeAssertion(node))) {
        return getInnerNode(node.expression, typescript);
    }
    else {
        return node;
...
function expressionContainsSuperKeyword(expression, typescript) {
    ...
    if (isSuperExpression(expression, typescript))
        ...
        return expressionContainsSuperKeyword(expression.expression, typescript);
    else if (((_a = typescript.isTypeAssertionExpression) === null || _a === void 0 ? void 0 : _a.call(typescript, expression)) || (!("isTypeAssertionExpression" in typescript) && typescript.isTypeAssertion(expression))) {
        return expressionContainsSuperKeyword(expression.expression, typescript);
    }
    else {
        return false;
...
function evaluatePropertyAccessExpression(options) {
    ...
    if (typeof match === "function" && statementTraversalStack.includes(typescript.SyntaxKind.CallExpression)) {
        return {
            [LAZY_CALL_FLAG]: 0 /* LiteralFlagKind.CALL */,
...
class AsyncIteratorNotSupportedError extends EvaluationError {
    constructor({ message = `It is not possible to evaluate an async iterator'`, typescript, environment }) {
        var _a, _b;
        super({ message, environment, node: (_b = (_a = typescript.factory) === null || _a === void 0 ? void 0 : _a.createEmptyStatement()) !== null && _b !== void 0 ? _b : typescript.createEmptyStatement() });
    }
}
...
function evaluateNode({ node, ...rest }) {
    ...
    else if (((_d = (_c = rest.typescript).isTypeAssertionExpression) === null || _d === void 0 ? void 0 : _d.call(_c, node)) || (!("isTypeAssertionExpression" in rest.typescript) && rest.typescript.isTypeAssertion(node))) {
...
class Logger {
    constructor(optionsOrLogLevel = {}) {
        const { logLevel = 0 /* LogLevelKind.SILENT */, color: { info = "white", verbose = "yellow", debug = "magenta" } = {} } = typeof optionsOrLogLevel === "object" ? optionsOrLogLevel : { logLevel: optionsOrLogLevel };
        this.options = {
            logLevel,
            color: {
                info,
     ...
     */
    logInfo(message) {
        if (this.options.logLevel < 1 /* LogLevelKind.INFO */)
            return;
     ...
     */
    logVerbose(message) {
        if (this.options.logLevel < 2 /* LogLevelKind.VERBOSE */)
            return;
     ...
     */
    logDebug(message) {
        if (this.options.logLevel < 3 /* LogLevelKind.DEBUG */)
            return;
     ...
     */
    compactValue(value) {
        return util.inspect(value, { depth: 0, colors: true, compact: true, maxArrayLength: 5 });
    }
 ...
 */
function evaluate({ typeChecker, node, environment: { preset = "NODE", extra = {} } = {}, moduleOverrides = {}, typescript = TSModule__namespace, logLevel = 0 /* LogLevelKind.SILENT */, policy: { deterministic = false, network = false, console = false, maxOps = Infinity, maxOpDuration = Infinity, io = {
    read: true,
    write: false
}, process = {
    exit: false,
...
exports.MaxOpDurationExceededError = MaxOpDurationExceededError;
exports.MaxOpsExceededError = MaxOpsExceededError;
exports.MissingCatchOrFinallyAfterTryError = MissingCatchOrFinallyAfterTryError;
exports.ModuleNotFoundError = ModuleNotFoundError;
exports.NetworkError = NetworkError;
exports.NonDeterministicError = NonDeterministicError;
exports.NotCallableError = NotCallableError;
exports.PolicyError = PolicyError;
exports.ProcessError = ProcessError;
exports.UndefinedIdentifierError = UndefinedIdentifierError;
exports.UndefinedLeftValueError = UndefinedLeftValueError;
exports.UnexpectedNodeError = UnexpectedNodeError;
exports.UnexpectedSyntaxError = UnexpectedSyntaxError;
