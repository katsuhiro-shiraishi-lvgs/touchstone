  ...
  return core.createMachine(
    ...
    },
    {
      guards: {
        ...
        isFirstOptionFocused: (ctx2) => combobox_dom.dom.getFirstEl(ctx2)?.id === ctx2.focusedId,
        isLastOptionFocused: (ctx2) => combobox_dom.dom.getLastEl(ctx2)?.id === ctx2.focusedId,
      ...
      activities: {
        syncInputValue: (ctx2) => {
          const inputEl = combobox_dom.dom.getInputEl(ctx2);
          if (!inputEl)
            return;
          return mutationObserver.observeAttributes(inputEl, ["data-value"], () => {
            inputEl.value = inputEl.dataset.value || "";
          });
        },
        trackInteractOutside(ctx2, _evt, { send }) {
          return interactOutside.trackInteractOutside(combobox_dom.dom.getInputEl(ctx2), {
            exclude(target) {
              const ignore = [combobox_dom.dom.getContentEl(ctx2), combobox_dom.dom.getTriggerEl(ctx2)];
        ...
        hideOtherElements(ctx2) {
          if (!ctx2.ariaHidden)
            return;
          return ariaHidden.ariaHidden([combobox_dom.dom.getInputEl(ctx2), combobox_dom.dom.getContentEl(ctx2), combobox_dom.dom.getTriggerEl(ctx2)]);
        },
        computePlacement(ctx2) {
          ctx2.currentPlacement = ctx2.positioning.placement;
          return popper.getPlacement(combobox_dom.dom.getControlEl(ctx2), combobox_dom.dom.getPositionerEl(ctx2), {
            ...ctx2.positioning,
            onComplete(data) {
        ...
        trackOptionNodes(ctx2, evt, meta) {
          ...
          exec();
          return mutationObserver.observeChildren(combobox_dom.dom.getContentEl(ctx2), exec);
        },
        scrollOptionIntoView(ctx2, _evt, { getState }) {
          const inputEl = combobox_dom.dom.getInputEl(ctx2);
          return mutationObserver.observeAttributes(inputEl, ["aria-activedescendant"], () => {
            const evt = getState().event;
            const isKeyboardEvent = /(ARROW_UP|ARROW_DOWN|HOME|END|TAB)/.test(evt.type);
            if (!isKeyboardEvent)
              return;
            const option = combobox_dom.dom.getActiveOptionEl(ctx2);
            option?.scrollIntoView({ block: "nearest" });
            if (ctx2.autoComplete) {
              combobox_dom.dom.focusInput(ctx2);
            }
          });
        }
      },
      actions: {
        setupLiveRegion(ctx2) {
          ctx2.liveRegion = liveRegion.createLiveRegion({
            level: "assertive",
            document: combobox_dom.dom.getDoc(ctx2)
          });
        },
        ...
        blurInputIfNeeded(ctx2) {
          if (ctx2.autoComplete || !ctx2.blurOnSelect)
            return;
          domQuery.raf(() => {
            combobox_dom.dom.getInputEl(ctx2)?.blur();
          });
        },
        focusInput(ctx2, evt) {
          if (evt.type === "CHANGE")
        ...
        scrollToTop(ctx2) {
          const listbox = combobox_dom.dom.getContentEl(ctx2);
          if (!listbox)
            return;
          listbox.scrollTop = 0;
        },
        ...
        invokeOnHighlight(ctx2) {
          ...
          const relatedTarget = combobox_dom.dom.getMatchingOptionEl(ctx2, value);
        ...
        highlightFirstOption(ctx2) {
          domQuery.raf(() => {
            setHighlight(ctx2, combobox_dom.dom.getFirstEl(ctx2));
          });
        },
        focusFirstOption(ctx2) {
          domQuery.raf(() => {
            setFocus(ctx2, combobox_dom.dom.getFirstEl(ctx2));
          });
        },
        focusLastOption(ctx2) {
          domQuery.raf(() => {
            setFocus(ctx2, combobox_dom.dom.getLastEl(ctx2));
          });
        },
        focusNextOption(ctx2) {
          domQuery.raf(() => {
            const option = combobox_dom.dom.getNextEl(ctx2, ctx2.focusedId ?? "");
            setFocus(ctx2, option);
          });
        },
        focusPrevOption(ctx2) {
          domQuery.raf(() => {
            const option = combobox_dom.dom.getPrevEl(ctx2, ctx2.focusedId ?? "");
            setFocus(ctx2, option);
          });
        },
        focusMatchingOption(ctx2) {
          domQuery.raf(() => {
            const option = combobox_dom.dom.getMatchingOptionEl(ctx2, ctx2.selectionData?.value);
            option?.scrollIntoView({ block: "nearest" });
            setFocus(ctx2, option);
          });
        },
        announceOptionCount(ctx2) {
          domQuery.raf(() => {
            const count = combobox_dom.dom.getOptionCount(ctx2);
            if (!count)
              return;
        ...
        setSectionLabel(ctx2) {
          const label = combobox_dom.dom.getClosestSectionLabel(ctx2);
          if (!label)
            return;
...
function setHighlight(ctx, option) {
  if (!option)
    return;
  const data = combobox_dom.dom.getOptionData(option);
