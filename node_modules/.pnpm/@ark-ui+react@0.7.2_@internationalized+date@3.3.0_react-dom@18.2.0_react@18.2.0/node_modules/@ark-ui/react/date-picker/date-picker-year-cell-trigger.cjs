...
const DatePickerYearCellTrigger = forwardRef.forwardRef((props, ref) => {
  const { getYearCellTriggerProps } = datePickerContext.useDatePickerContext();
  const cellProps = datePickerCellContext.useDatePickerCellContext();
  const mergedProps = react.mergeProps(getYearCellTriggerProps(cellProps), props);
