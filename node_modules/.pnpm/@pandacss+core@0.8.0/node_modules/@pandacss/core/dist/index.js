...
__export(src_exports, {
  ...
  discardDuplicate: () => discardDuplicate,
...
var AtomicRule = class {
  constructor(context) {
    ...
    this.root = import_postcss4.default.root();
  }
  root;
  layer = "utilities";
  ...
  process = (options) => {
    ...
    const styleObject = (0, import_shared4.normalizeStyleObject)(styles, this.context);
    ...
    (0, import_shared4.walkObject)(styleObject, (value, paths) => {
      ...
      if (transformed.layer) {
        const atRule4 = import_postcss4.default.atRule({
          name: "layer",
    ...
    if (this.root.nodes.length === 0)
      return;
    const atRule3 = import_postcss4.default.atRule({
      name: "layer",
      params: this.layer,
      nodes: [this.root]
    });
...
var Breakpoints = class {
  ...
  get ranges() {
    ...
    const values = breakpoints.flatMap((_name, index) => {
      ...
      return [up, only, down];
    }).filter(([_, value]) => value !== "").concat(permuations.map(([min, max]) => [`${min}To${(0, import_shared5.capitalize)(max)}`, this.between(min, max)]));
...
function sortBreakpoints(breakpoints) {
  return Object.entries(breakpoints).sort(([, minA], [, minB]) => {
    ...
    if (max != null) {
      const computedMax = parseFloat((0, import_shared5.toPx)(max) ?? "") - 0.05;
      max = (0, import_shared5.toEm)(`${computedMax}px`);
    }
...
function serializeStyle(styleObj, context) {
  ...
  const result = {};
  (0, import_shared6.walkObject)(styleObj, (value, paths) => {
...
function optimizeCss(code, options = {}) {
  ...
  const { css } = (0, import_postcss8.default)([
...
function expandCssFunctions(code, options = {}) {
  const { token } = options;
  const { css } = (0, import_postcss8.default)([expand_token_fn_default(token)]).process(code);
  return css;
}
function discardDuplicate(code) {
  const { css } = (0, import_postcss8.default)([mergeLayers(), (0, import_postcss_discard_duplicates.default)(), sortMediaQueries(), sortCss(), prettify()]).process(code);
  return css;
}
function expandNestedCss(code) {
  const { css } = (0, import_postcss8.default)([(0, import_postcss_nested2.default)(), prettify()]).process(code);
  return css;
}
function prettifyCss(code) {
  const { css } = (0, import_postcss8.default)([prettify()]).process(code);
  return css;
}
...
function mergeCss(oldCss, newCss) {
  ...
  const oldUtilities = oldRoot.nodes.filter(
    (0, import_ts_pattern2.isMatching)({
      type: "atrule",
      name: "layer",
      params: "utilities"
    })
...
var Recipes = class {
  ...
  assignRecipe = (name, recipe) => {
    ...
    const jsx = recipe.jsx ?? [(0, import_shared9.capitalize)(name)];
    ...
    sharedState.configs.set(name, {
      ...
      config: recipe,
      splitProps: (props) => {
        return (0, import_shared9.splitProps)(props, variantKeys);
      }
    });
  };
  assignRules = () => {
  ...
  getNames = (0, import_shared9.memo)((name) => {
    return {
      upperName: (0, import_shared9.capitalize)(name),
      dashName: (0, import_shared9.dashCase)(name),
      jsxName: (0, import_shared9.capitalize)(name)
    };
  });
  getRecipe = (0, import_shared9.memo)((name) => {
    return sharedState.configs.get(name);
  });
  getConfig = (0, import_shared9.memo)((name) => {
    return this.recipes[name];
  });
  find = (0, import_shared9.memo)((jsxName) => {
    return this.details.find((node) => node.match.test(jsxName));
  });
  filter = (0, import_shared9.memo)((jsxName) => {
  ...
  normalize = (config) => {
    const {
      name,
      jsx = [(0, import_shared9.capitalize)(name)],
      base = {},
      variants = {},
      defaultVariants = {},
      description = "",
...
function extractParentSelectors(selector) {
  ...
  (0, import_postcss_selector_parser.default)((selectors) => {
    selectors.each((selector2) => {
      ...
      (0, import_ts_pattern3.match)(condition).with({ type: "parent-nesting" }, () => {
...
${css}`;
      ...
      if (error instanceof import_postcss9.CssSyntaxError) {
...
var Utility = class {
  ...
  getPropertyValues = (config, resolveFn) => {
    ...
    if ((0, import_shared10.isString)(values)) {
    ...
    if ((0, import_shared10.isFunction)(values)) {
  ...
  defaultTransform = (0, import_shared10.memo)((value, prop) => {
  ...
  setClassName = (property, raw) => {
    ...
    if (!config || !config.className) {
      className = this.hash((0, import_shared10.hypenateProperty)(property), raw);
    } else {
  ...
  transform = (prop, value) => {
    ...
    return (0, import_shared10.compact)({
      layer: this.configs.get(key)?.layer,
      className: this.getOrCreateClassName(key, (0, import_shared10.withoutSpace)(value)),
